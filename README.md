...
- Fills `.github/pull_request_template.md` when the host repository provides one, using explicit watcher markers or conventional Problem/Changes/Agent context sections; unrecognized templates retain an appended watcher summary.
...
`sweep` mode searches open issues with `sweep-query`, processes at most `max-sweep-items`, and is intended to run with `allow-fix: 'false'` and `allow-close: 'false'` unless explicitly testing on a disposable repository. By default (`skip-sweep-trusted-authors: 'true'`), `auto`, `triage`, `investigate`, and `sweep` modes skip issues created by trusted repository authors (`OWNER`, `MEMBER`, or `COLLABORATOR`) before posting comments, calling pi, or changing labels. This includes direct issue events and queued issues when the worker processes them. Because GitHub can hide private org membership in `author_association`, these modes also fall back to the repository collaborator permission API and skip authors with `triage`, `write`, `maintain`, or `admin` permission. Explicit watcher commands and `mode: fix` bypass this filter. Set `skip-sweep-trusted-authors: 'false'` on the processing workflow (the worker when using a queue) to include trusted-author issues. The input retains its original name for compatibility.
...
| `skip-sweep-trusted-authors` | `true` | In `auto`, `triage`, `investigate`, and `sweep` modes, skip issues created by trusted repository authors (`OWNER`, `MEMBER`, or `COLLABORATOR`), with a collaborator-permission fallback for private org memberships. Includes queued issues at processing time. Explicit watcher commands and `mode: fix` bypass this filter. |
