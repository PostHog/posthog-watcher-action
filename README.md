...
- Fix mode removes GitHub/OpenAI/secrets-like variables from the `pi` subprocess environment, passes the OpenAI key via the pi CLI, and disables the agent `bash` tool. Wrapper-owned validation commands still run outside pi.
