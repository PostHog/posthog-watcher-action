...
export type Mode = QueuedMode | 'sweep' | 'enqueue' | 'drain-queue';
...
function normalizeMode(value: string): Mode {
  if (value === 'auto' || value === 'triage' || value === 'investigate' || value === 'fix' || value === 'sweep' || value === 'enqueue' || value === 'drain-queue') {
    return value;
  }
  throw new Error('mode must be one of: auto, triage, investigate, fix, sweep, enqueue, drain-queue');
}
